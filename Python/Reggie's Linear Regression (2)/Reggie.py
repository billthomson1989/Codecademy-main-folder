...
def calculate_error(m, b, point):
    ...
    diff_y_value_y_point = abs(y_value - y_point)
    return diff_y_value_y_point
...
# 
# In[14]:
possible_ms = [m*0.1 for m in range(-100,100)] #your list comprehension here
#possible_ms = [round(m*0.1,2) for m in range(-100,100)]
#print(possible_ms)
# Now, let's make a list of `possible_bs` to check that would be the values from -20 to 20 inclusive, in steps of 0.1:
# In[17]:
possible_bs = [b*0.1 for b in range(-20, 20)]#your list comprehension here
print(possible_bs)
...
for i in possible_ms:
    for j in possible_bs:
        a = calculate_all_error(i,j, datapoints)
        if a < smallest_error:
            smallest_error = round(a,2)
            bestm = round(i,2)
            bestb = round(j,2)
print(bestm)
print(bestb)
